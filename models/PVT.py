...
# def test_pvt_huge_v2(num_classes: int = 1000, **kwargs):
#     # trained ImageNet-1K
#     # Example layer configuration for EfficientNetV2
#     model = pvt_huge_v2()
#     return model
#
...
# model = pvt_tiny().cuda()
# model.eval()
# weights_path = r"D:\2self\AgriPTM-learning\weights\PVT\pvt_tiny.pth"
# msg=model.load_state_dict(torch.load(weights_path), strict=False)
# print(msg)

# if __name__ == "__main__":
#     # 测试输入
#     test_input = torch.randn(1, 3, 224, 224).cuda()  # 创建一个随机输入张量
#     output = model(test_input)  # 前向传播
#     print("Output shape:", output.shape)  # 输出结果的形状
...
if __name__ == '__main__':
    ...
    net = pvt_small(num_classes=3, img_size=512).cuda()
