...
class VCNUsBlock(nn.Module):
    ...
    def forward(self, x, memory):
        ...
        if isinstance(self.gamma_1, nn.Parameter):
            ...
            x = x + self.drop_path(self.gamma_2.unsqueeze(-1).unsqueeze(-1) * self.mlp(self.norm2(x)))
        else:
...
class VisionCognitiveModel(nn.Module):
    def __init__(self,
                 filter_strategy1 = 23,
                 filter_strategy2 = 7,
                 use_layerscale=False,
...
def vcm_tiny(num_classes: int = 1000, **kwargs):
...
if __name__ == '__main__':
    ...
    net = vcm_tiny(num_classes=3).cuda()
    import torchsummary
