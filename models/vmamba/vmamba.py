...
# =====================================================
def vanilla_vmamba_tiny(num_classes=1000):
    return VSSM(
...
# =====================================================
def vmamba_tiny_s1l8(channel_first=True, num_classes=1000):
    return VSSM(
...
def vmamba_small_s1l20(channel_first=True, num_classes=1000):
    return VSSM(
...
def vmamba_base_s1l20(channel_first=True, num_classes=1000):
    return VSSM(
...
if __name__ == "__main__":
    model_ref = vanilla_vmamba_tiny(num_classes=3)

    model = VSSM(
        depths=[2, 2, 4, 2], dims=96, drop_path_rate=0.2, 
        patch_size=4, in_chans=3, num_classes=96, 
        ssm_d_state=64, ssm_ratio=1.0, ssm_dt_rank="auto", ssm_act_layer="gelu",
        ssm_conv=3, ssm_conv_bias=False, ssm_drop_rate=0.0, 
        ssm_init="v2", forward_type="m0_noz", 
        mlp_ratio=4.0, mlp_act_layer="gelu", mlp_drop_rate=0.0, gmlp=False,
    ...
    print(parameter_count(model_ref)[""])
    print(model_ref.flops()) # wrong
    # model.cuda().train()
...
#     net = vanilla_vmamba_tiny(num_classes=1000).cuda()
#     import torchsummary
