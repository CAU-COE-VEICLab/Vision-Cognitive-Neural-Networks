...
class VCNU_SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, memory_dim, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 fused_window_process=False, ab_norm_attn=True, ab_norm_ltm=False, model_style='trans', training_mode='tfs',
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, memory):
        ...
        _, _, memory_dim = memory.shape
        if self.model_style == 'conv':
            memory = self.memory_pooling(memory)
        else:
            memory = self.memory_pooling(memory.view(B, H, W, memory_dim).permute(0, 3, 1, 2)).flatten(2).transpose(1, 2).contiguous()
        return x, memory
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, memory_dim, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,
                 fused_window_process=False, ab_norm_attn=True, ab_norm_ltm=False, model_style='trans', training_mode='tfs', use_layerscales=False, layer_scale_init_value=1e-6):
        ...
        self.blocks = nn.ModuleList([
            VCNU_SwinTransformerBlock(ab_norm_attn=ab_norm_attn,ab_norm_ltm=ab_norm_ltm,
    ...
    def forward(self, x, memory):
        ...
        if self.downsample is not None:
            x, memory = self.downsample(x, memory)
        return x, memory
...
class VCNU_SwinTransformer(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, use_memory_embedding=False,
                 is_efficient_finetune=False, filter_strategy1=18, filter_strategy2=6, pretrain_image_size=224,
                 ab_norm_attn=True, ab_norm_ltm=False, model_style='trans', training_mode='tfs',
                 use_layerscales=False,  layer_scale_init_value=1e-6,
        ...
        # is build lora style vcnu-memory
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(use_layerscales=use_layerscales, layer_scale_init_value=layer_scale_init_value,
                               ab_norm_attn=ab_norm_attn,ab_norm_ltm=ab_norm_ltm,
                               model_style=model_style,training_mode=training_mode,
                               memory_dim=self.memory_dim,
...
def test_SYNA(num_classes: int = 1000, **kwargs):
    ...
    model = VCNU_SwinTransformer(
        ...
        use_layerscales=False, layer_scale_init_value=1e-6,
        filter_strategy1=23, filter_strategy2=7, pretrain_image_size=224,
        ab_norm_attn=True, ab_norm_ltm=False, model_style='trans', training_mode='efficient_ft',  # efficient_ft
        img_size=224, patch_size=4, in_chans=3, num_classes=1000,
...
def freeze_transferlearning(model):
