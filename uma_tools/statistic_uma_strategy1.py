            ...
            if int(root.split('/')[-1].split('n')[-1]) >= start_index and  int(root.split('/')[-1].split('n')[-1]) <= end_index:
                ...
                for file in files:
                    if file.endswith(('.PNG', '.JPG', '.JPEG')):
                        ...
                        assert image_tensor.shape == target_size, f"{root}/{file}  image size= {image_tensor.shape}"

                        filter_size_list, threshold_value, ssim_origin_dic, ssim_percentage_dic = countssim(image_tensor, (target_size[0]))
                ...
                for i in range(3, target_size[0] // 2 + 1):
